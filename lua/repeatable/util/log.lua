...
function log.notify(scope, level, verbose, str, ...)
    if not verbose and _G.Repeatable.config ~= nil and not _G.Repeatable.config.debug then
        return
    end
    ...
    vim.notify(
        string.format("[repeatable.nvim@%s] %s", scope, string.format(str, ...)),
        level,
        { title = "repeatable.nvim" }
    )
end
...
function log.warn_deprecation(options)
    ...
    if uses_deprecated_option then
        ...
        log.notify(
            ...
            vim.log.levels.WARN,
            true,
            "use `:h Repeatable.options` to read more."
        )
    end
end
