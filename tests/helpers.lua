...
Helpers.expect.buf_width = MiniTest.new_expectation(
    ...
    function(child, field, value)
        return Helpers.expect.equality(
            child.lua_get("vim.api.nvim_win_get_width(_G.Repeatable.state." .. field .. ")"),
            value
        )
    end,
    error_message
...
Helpers.expect.config = MiniTest.new_expectation(
    ...
    function(child, field, value)
        if field == "" then
            return Helpers.expect.global(child, "_G.Repeatable.config" .. field, value)
        else
            return Helpers.expect.global(child, "_G.Repeatable.config." .. field, value)
        end
    end,
...
Helpers.expect.config_type = MiniTest.new_expectation(
    ...
    function(child, field, value)
        return Helpers.expect.global(child, "type(_G.Repeatable.config." .. field .. ")", value)
    end,
    error_message
)

Helpers.expect.state = MiniTest.new_expectation("state matches", function(child, field, value)
    return Helpers.expect.global(child, "_G.Repeatable.state." .. field, value)
end, error_message)

Helpers.expect.state_type = MiniTest.new_expectation(
    "state type matches",
    function(child, field, value)
        return Helpers.expect.global(child, "type(_G.Repeatable.state." .. field .. ")", value)
    end,
    error_message
)
...
Helpers.new_child_neovim = function()
    ...
    child.nnp = function()
        child.cmd("Repeatable")
        child.wait()
    end

    child.get_wins_in_tab = function(tab)
        tab = tab or "_G.Repeatable.state.active_tab"
